...
class CRF_model(object):
	...
	def __init__(self,  We_initial, params):
		...
		self.eval_fn = theano.function([input_var, target_var, mask_var, mask_var1, length], loss_train, on_unused_input='ignore')
		...
		testacc = 0
        	...
        	try:
				...
				bb = 0
					...
					#print y0
					print 'start new minibatch'
					x00, x0mask, x0mask1, y00, maxlen = self.prepare_data(x0, y0)					
					#start_time = time.time()
					print 'process the data'
					cost = self.eval_fn(x00, y00, x0mask, x0mask1 , maxlen)
					print 'eval cost', cost

                 			cost, _, _, _ = self.train_fn(x00, y00, x0mask, x0mask1 , maxlen)
					
					#end_time = time.time()
					#print('train', end_time-start_time)
					print y00.shape[1],   cost
				...
				print 'finish traning one more epoch'
				...
				dev_kf = get_minibatches_idx(len(devX), 10)
				devpred = 0
				devnum = 0
				print 'start eva'
				for _, dev_index in dev_kf:
					...
					print 'start eva minibatch', devy0
					loss, devpred0, devnum0, _   = self.eval_fn(devx0, devy0, devx0mask, devx0mask1, devmaxlen)
					print 'end eva minibatch loss', loss
					devpred += devpred0
					devnum += devnum0
				...
				if bestdev < devacc:
					...
					test_kf = get_minibatches_idx(len(testX), 10)
                                	testpred = 0
                                	testnum = 0
