...
class base_model(object):
	...
	if (params.inf==0):
                        ...
                        l_cnn_1 = lasagne.layers.Conv1DLayer(l_cnn_input, hidden, 3, 1, pad = 'same')
                        l_cnn_3 = lasagne.layers.Conv1DLayer(l_cnn_input, hidden, 1, 1, pad = 'same')
			...
			l_cnn_input = lasagne.layers.DimshuffleLayer(l_emb_word, (0, 2, 1))
                        l_cnn = lasagne.layers.Conv1DLayer(l_cnn_input, hidden, 5, 1, pad = 'same')
			concat2 = lasagne.layers.DimshuffleLayer(l_cnn, (0, 2, 1))
	...
	final_pred = T.argmax(test_output_1, axis=1)
        #final_pred = final_pred.reshape((-1, g.shape[0]))
 	...
 	#y1 = T.ones_like(y)
        #SUM = y1.sum(dtype=theano.config.floatX)
        acc = 1.0 * corr_test/num_tokens

	self.acc_function = theano.function([g, char_input_var, gmask, y, idxs, length], acc, on_unused_input='warn')	
	...
	self.train_function = theano.function([g, char_input_var, gmask, y, idxs, length], [cost, acc], updates=updates, on_unused_input='warn')	
	...
	counter = 0
        ...
        try:
		    ...
		    start_time = time.time()    
                    traincost, trainacc = self.train_function(x0, x0_char, x0mask, y0, idx0, length)
		    end_time = time.time()
		    ##print(end_time-start_time)
		start_time = time.time()
                devacc = self.acc_function(devx0, devx0_char, devx0mask, devy0, devidx, devlength)
		end_time = time.time()
                ##print('inference', end_time-start_time)
                start_time = time.time()
                testacc = self.acc_function(testx0, testx0_char, testx0mask, testy0, testidx, testlength)
                end_time = time.time()
                ##print('inference', end_time-start_time)
