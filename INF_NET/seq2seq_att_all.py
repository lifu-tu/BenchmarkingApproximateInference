...
class Seq2Seq(object):
	def __init__(self, We, params):
		...
		encoderMask_shuffle = encoderMask.dimshuffle(1,'x',0)
		...
		self.params += [self.linear, self.de_lookuptable, self.hidden_decode]    #the initial hidden state of decoder lstm is zeros
		...
		att_weight = tensor.batched_dot(state_below.dimshuffle(1, 0, 2), Encoder_shuffle_re.dimshuffle(0,2,1))
		att_weight = tensor.nnet.softmax(att_weight.reshape((-1, encoderInputs.shape[0]))).reshape((-1, encoderInputs.shape[0], encoderInputs.shape[0]))
		
		att_weight = att_weight*encoderMask_shuffle
		
		att_weight =  att_weight/(tensor.sum(att_weight, axis=2).dimshuffle(0,1, 'x') + eps)
		
		#### att_weight :(B, L, L)        Encoder_shuffle: (B, L, h)	
		ctx = 	tensor.batched_dot(att_weight, Encoder_shuffle)
		ctx = ctx.dimshuffle(1,0,2)
	
		decoder_lstm_outputs = tensor.concatenate([state_below, ctx], axis=2)
		...
		###### always use privous predict as next input 
			...
			#### set att weight outside of the mask to 0
			attn_weight = attn_weight*(encoderMask.dimshuffle(1,0))
			attn_weight = attn_weight/(tensor.sum(attn_weight, axis=1).dimshuffle(0,'x') + eps)
			####### ctx_ : (b, h) 			
			ctx_ = tensor.sum(Encoder_shuffle*attn_weight[:,:, None], axis=1)
			###ctx_ = ctx_.dimshuffle(1,0)			

			state_below0 = tensor.concatenate([state_below0, ctx_], axis =1)
