...
class CRF_seq2seq_model(object):
	...
	def __init__(self,  We_initial, params):
		...
		self.params += [self.linear, self.de_lookuptable, self.linear_bias]    #concatenate
	...
	def train(self, trainX, trainY, devX, devY, testX, testY, params):	
		...
		tagger_keys = params.tagger.keys()
        	...
        	try:
				...
				devloss, devloss0, devpred, devnum, devpred   = self.eval_fn(devx0, devy0, devx0mask, devx0mask1, devmaxlen, devx1_in2)
				...
				f = open('pred_ner_inf_' + params.outfile, 'w')
                                ...
                                for ii, devl in enumerate(devlength):
                                        for jj in range(devl):
                                                eva_string = params.devrawx[ii][jj] + ' '+ params.devpos[ii][jj] + ' '+ params.taggerlist[devy0[ii, jj]] +' ' +  params.taggerlist[devpred[ii, jj]]
				...
				if bestdev < dev_f10:
					...
					testloss, _, testpred, testnum, testpred = self.eval_fn(testx0, testy0, testx0mask, testx0mask1, testmaxlen, testx1_in2)
					
					f = open('pred_ner_inf_' + params.outfile, 'w')
                                        testlength = [len(s) for s in testX]
                                        for ii, testl in enumerate(testlength):
                                                for jj in range(testl):
                                                        eva_string = params.testrawx[ii][jj] + ' '+ params.testpos[ii][jj] + ' '+ params.taggerlist[testy0[ii, jj]] +' ' +  params.taggerlist[testpred[ii, jj]]
