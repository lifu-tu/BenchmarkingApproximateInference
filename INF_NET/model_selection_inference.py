...
class CRF_model(object):
	...
	def train(self, trainX, trainY, devX, devY, testX, testY, params):	
		...
		tdevx0, tdevx0mask, tdevx0mask1, tdevy0, tdevmaxlen = self.prepare_data(devX[:10], devY[:10])	
        	...
        	try:
				...
				#bb = 0
					...
					x0, x0mask, x0mask1, y0, maxlen = self.prepare_data(x0, y0)					
                 			...
                 			cost, regu_cost = self.train_fn(x0, y0, x0mask, x0mask1, maxlen, eidx)
					#print regu_cost
				...
				start_time2 = time.time()
				_, _, _, _, dev_pred   = self.eval_fn(tdevx0, tdevy0, tdevx0mask, tdevx0mask1, tdevmaxlen)
				end_time2 = time.time()
				
				self.textfile.write("inference time (dev): %f \n" % (end_time2-start_time2))
