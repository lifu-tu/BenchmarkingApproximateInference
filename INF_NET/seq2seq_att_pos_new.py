...
class Seq2Seq(object):
	def __init__(self, We, params):
		...
		##self.hidden_decode = theano.shared(name="Hidden to Decode", value= init_xavier_uniform(2*self.en_hidden_size, self.de_hidden_size), borrow = True)
		...
		state_below = self.lookuptable[encoderInputs.flatten()].reshape((encoderInputs.shape[0], encoderInputs.shape[1], num_inputs))
		...
		bestdev = -1
				...
				devacc= 1.0*np.multiply(np.equal(devpred, devy1), devx0mask).sum()/devx0mask.sum()			
					...
					self.textfile.write("epoch %d  devacc %f  testacc %f\n" %(eidx , devacc, testacc))
                                	...
                                	print 'prediction', devpred[:, 0] , 'dev acc ', devacc, 'test acc ', testacc
