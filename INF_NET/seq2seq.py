...
class Seq2Seq(object):
	def __init__(self, We, params):
		...
		self.de_lookuptable = theano.shared(name="Decoder LookUpTable", value=init_xavier_uniform(self.num_labels +1, self.de_hidden_size), borrow=True)
                ...
                self.linear = theano.shared(name="Linear", value = init_xavier_uniform(self.de_hidden_size , self.num_labels), borrow= True)
		...
		for _ in range(self.lstm_layers_num):
			...
			self.hos += tensor.tanh(tensor.dot(hs[-1], self.hidden_decode) + self.hidden_bias),
			self.Cos += tensor.tanh(tensor.dot(Cs[-1], self.hidden_decode) + self.hidden_bias),
			state_below = hs
		...
		###### always use privous predict as next input 
			...
			msk_ = msk_.dimshuffle('x', 0)
                        ...
                        newpred = tensor.dot(state_below0, self.linear).reshape((encoderInputs.shape[1], self.num_labels))
