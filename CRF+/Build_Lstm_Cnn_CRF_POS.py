...
class CRF_model(object):
	...
	def prepare_data(self, seqs, labels, char_seqs):
		lengths = [len(s) for s in seqs]
                ...
                char_len = [len(s0) for s in char_seqs for s0 in s]
                char_maxlen = np.max(char_len)
	...
	def __init__(self,  We_initial, char_embedd_table_initial, params):
		...
		local_energy_eval = local_energy_eval.reshape((-1, length, 25))
	...
	def train(self, train, dev, test, params):	
		...
		start_time = time.time()
        	...
        	try:
				...
				start_time = time.time()
				devpred, devnum, devpredtion = self.eval_fn(devx0, devx0_char, devy0, devx0mask, devx0mask1,devmaxlen)
				end_time = time.time()
				...
				if bestdev < devacc:
					bestdev = devacc
					best_t = eidx
                                        testpred, testnum, _ = self.eval_fn(testx0, testx0_char, testy0, testx0mask, testx0mask1, testmaxlen)
